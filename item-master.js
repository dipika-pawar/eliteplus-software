document.addEventListener("DOMContentLoaded", () => {
  ...
  const UNIT_API_URL = "https://eliteplus-software-backend.vercel.app/api/unit";
  const TAX_API_URL = "https://eliteplus-software-backend.vercel.app/api/tax";
  ...
  if (menuToggle && sidebar && toggleIcon) {
    menuToggle.addEventListener("click", (e) => {
      ...
      e.stopPropagation();
      toggleIcon.className = sidebar.classList.contains("open")
        ? "fa-solid fa-xmark"
        : "fa-solid fa-bars";
    });
  }
  ...
  const openAddUnitModalEditBtn = document.getElementById(
    "openAddUnitModalEditBtn",
  );
  ...
  const openAddTaxModalEditBtn = document.getElementById(
    "openAddTaxModalEditBtn",
  );
      ...
      try {
        ...
        if (response.ok || response.status === 201) {
          ...
          if (targetUnitDropdown) {
            ...
            clearError(
              targetUnitDropdown,
              targetUnitDropdown.id === "itemUnit"
                ? "itemUnitError"
                : "editUnitError",
            );
          }
          ...
          showError(
            newUnitNameInp,
            "newUnitNameError",
            result.message || "Error saving unit.",
          );
        }
      } catch (err) {
        addOptionToSelect(unitInp, unitVal);
        ...
        if (
          !savedUnits.some((u) => u.toLowerCase() === unitVal.toLowerCase())
        ) {
    ...
    btnSaveTax.addEventListener("click", async () => {
      ...
      if (taxVal === "") {
        showError(
          newTaxNameInp,
          "newTaxNameError",
          "Please enter a tax category name.",
        );
        return;
      }
      ...
      try {
        ...
        if (response.ok || response.status === 201) {
          ...
          if (targetTaxDropdown) {
            ...
            clearError(
              targetTaxDropdown,
              targetTaxDropdown.id === "itemTaxCategory"
                ? "itemTaxCategoryError"
                : "editTaxCategoryError",
            );
          }
          ...
          showError(
            newTaxNameInp,
            "newTaxNameError",
            result.message || "Error saving tax category.",
          );
        }
      } catch (err) {
        addOptionToSelect(taxInp, taxVal);
        ...
        addTaxModal.style.display = "none";
        alert(
          "Server is offline. Tax Category added temporarily to local memory.",
        );
      }
    });
  }
  ...
  function autoCalculateEditStockValue() {
    ...
    if (qty >= 0 && pPrice >= 0)
      editStockValInpEl.value = (qty * pPrice).toFixed(2);
  }
  if (editStockInpEl)
    editStockInpEl.addEventListener("input", autoCalculateEditStockValue);
  if (editPurchaseInpEl)
    editPurchaseInpEl.addEventListener("input", autoCalculateEditStockValue);
  ...
  function validateMainForm() {
    ...
    if (nameInp.value.trim() === "") {
      showError(nameInp, "itemNameError", "Item Name is required.");
      isValid = false;
    } else clearError(nameInp, "itemNameError");
    if (typeInp.value === "") {
      showError(typeInp, "itemTypeError", "Item Type is required.");
      isValid = false;
    } else clearError(typeInp, "itemTypeError");
    if (groupInp.value === "") {
      showError(groupInp, "itemGroupError", "Group / Category is required.");
      isValid = false;
    } else clearError(groupInp, "itemGroupError");
    if (unitInp.value === "") {
      showError(unitInp, "itemUnitError", "Unit selection is required.");
      isValid = false;
    } else clearError(unitInp, "itemUnitError");
    if (taxInp.value === "") {
      showError(taxInp, "itemTaxCategoryError", "Tax Category is required.");
      isValid = false;
    } else clearError(taxInp, "itemTaxCategoryError");
    if (hsnInp.value.trim() === "") {
      showError(hsnInp, "itemHsnCodeError", "HSN / SAC Code is required.");
      isValid = false;
    } else clearError(hsnInp, "itemHsnCodeError");
  ...
  itemForm.addEventListener("submit", async (e) => {
    ...
    if (itemImgInput.files[0])
      formData.append("itemImg", itemImgInput.files[0]);
    if (itemPdfInput.files[0])
      formData.append("itemPdf", itemPdfInput.files[0]);
  ...
  window.openEditModal = (id) => {
    ...
    if (editStockEl) {
      editStockEl.value = item.opening_stock_qty || item.current_stock || "0";
    }

    const editStockValEl = document.getElementById("editOpeningStockValue");
    if (editStockValEl) {
      editStockValEl.value = item.opening_stock_value || "0.00";
    }
    ...
    if (editPdfLabel) {
      editPdfLabel.innerHTML =
        '<i class="fa-solid fa-file-pdf"></i> Choose New PDF';
      editPdfLabel.style.borderColor = "";
    }
    if (existingPdfLink) {
      if (item.pdf_path && item.pdf_path !== "-") {
        const pdfUrl = item.pdf_path.startsWith("http")
          ? item.pdf_path
        ...
        existingPdfLink.innerHTML =
          '<span style="color:#64748b;">No existing PDF attached.</span>';
      }
    }
  ...
  editForm.addEventListener("submit", async (e) => {
    ...
    const stock = document.getElementById("editStock")
      ? document.getElementById("editStock").value.trim()
      : "0";
    const purchasePrice = document
      .getElementById("editPurchasePrice")
      .value.trim();
    ...
    if (editImgInput && editImgInput.files[0])
      formData.append("itemImg", editImgInput.files[0]);
    if (editPdfInput && editPdfInput.files[0])
      formData.append("itemPdf", editPdfInput.files[0]);
    ...
    try {
      ...
      if (idx !== -1) {
        items[idx] = {
          ...
          sales_price: price,
          mrp: mrp,
          packing_dimension: packing,
          video_link: videoLink,
          item_specification: description,
          image_path: image,
        };
        renderTable(items);
  ...
  function resetLabels() {
    ...
    document
      .querySelectorAll(".input-field-custom")
      .forEach((x) => x.classList.remove("invalid-field"));
  }

  closeModalBtn.addEventListener("click", () => {
    editModal.style.display = "none";
  });
  document.getElementById("btnNewClear")?.addEventListener("click", (e) => {
    e.preventDefault();
    itemForm.reset();
    resetLabels();
    nameInp.focus();
  });
  document.getElementById("addNewDetailsBtn")?.addEventListener("click", () => {
    document
  ...
  if (searchInput) {
    searchInput.addEventListener("input", () => {
      ...
      const filtered = items.filter((item) =>
        item.item_name.toLowerCase().startsWith(filter),
      );
      ...
      reader.onload = (event) => {
        try {
          ...
          importedData.forEach((item) => {
            const nameVal =
              item["Item Name"] || item.name || item.Name || "N/A";
            const codeVal = item.Code || item.code || "";
            const printNameVal =
              item["Print Name"] || item.printName || nameVal;
            const hsnValImport =
              item["HSN / SAC"] ||
              item["HSN / SAC Code"] ||
              item.hsn ||
            ...
            const nameExists = items.some(
              (x) => x.item_name.toLowerCase() === nameVal.toLowerCase(),
            );
            const codeExists =
              codeVal &&
              items.some(
                (x) =>
                  x.item_code &&
                  x.item_code.toLowerCase() ===
                    codeVal.toString().toLowerCase(),
              );
            ...
            if (!nameExists && !codeExists) {
              items.push({
                ...
                tax_category: taxVal,
                opening_stock_qty:
                  item["Opening Stock Qty"] || item.openingStock || "0",
                opening_stock_value:
                  item["Opening Stock Value"] || item.stockValue || "0.00",
                purchase_price:
                  item["Purchase Price"] || item.purchasePrice || "0",
                sales_price: (
                  item["Sales Price"] ||
                  item.price ||
                  item.Price ||
                ...
                packing_dimension:
                  item["Packing Dimension"] || item.packing || "",
                video_link: item["Video Link"] || item.videoLink || "",
                item_specification:
                  item["Item Specification"] || item.description || "",
  ...
  if (exportBtn) {
    exportBtn.addEventListener("click", () => {
      ...
      const exportCleanList = items.map((item) => ({
        ...
        "Opening Stock Qty": item.opening_stock_qty,
        "Opening Stock Value": item.opening_stock_value,
        "Purchase Price": item.purchase_price,
        "Sales Price": item.sales_price,
        MRP: item.mrp,
        "Packing Dimension": item.packing_dimension,
        "Video Link": item.video_link,
        "Item Specification": item.item_specification,
